...
class TestScaffold(AbstractTestScaffold):
    ...
    def run_test_case(self, test_case, environment):
        ...
        try:
            ...
            if expect_failure:
                # TODO: decide if you want to request line numbers.
                # if not, change the following two lines to be:
                #
...
def __generate_test_case_structure(
    ...
    # TODO: decide the file extensions for source code (.brewin), stdin (.in), and output (.exp)
    return [
        {
            "name": f"{category} | {i}",
...
def __generate_test_suite(version, successes, failures):
    # TODO: change the directories, if desired
